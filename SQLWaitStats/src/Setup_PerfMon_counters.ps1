...
\PhysicalDisk(*)\Avg. Disk Bytes/Transfert
...
if ($Ans -eq "yes"){
	...
	try{
		...
		Write-Output "DataCollectorSet stoped"
		Start-Sleep -s 2
		$datacollectorset.delete()
		Write-Output "DataCollectorSet deleted"
		Start-Sleep -s 2
	} catch {
	...
	if ($(get-odbcdsn | where name -eq $cntrname) ){
		Write-Output "DSN already exists ; trying to remove and then recreate"
		Remove-OdbcDsn -Name $cntrname -DsnType "System"
	}
