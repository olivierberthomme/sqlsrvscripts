...
from [master].[monitoring].[wait_stats] start_sp inner join 
	 [master].[monitoring].[wait_stats] end_sp on start_sp.wait_type=end_sp.wait_type
WHERE     CONVERT(nvarchar(MAX), start_sp.date, 120) like '2014-10-27 11%'
      and CONVERT(nvarchar(MAX), end_sp.date  , 120) like '2014-10-27 14%'
...
SELECT start_sp.wait_type
  ...
  FROM [master].[monitoring].[wait_stats] start_sp  full outer join
	   [master].[monitoring].[wait_stats] end_sp on start_sp.wait_type=end_sp.wait_type,
	   sum_wait
