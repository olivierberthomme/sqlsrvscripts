	...
	while ($threads -le $MaxThreads ){
		...
		while ($outstandings -le $MaxOutstandings){
			...
			if($result){
				...
				#Write-Output $dataheatMap_data.value
				generate_HTML
				$data_JS_heatMap_data.value = $($data_JS_heatMap_data.value -replace ".{1}$") + ","
			}
...
$output_file = $($scriptPath + "\DiskBenchmark_MSSQL_output.html")
...
benchmark $DRA_param ([ref]$dataDRA_latency) ([ref]$dataDRA_iops) ([ref]$dataDRA_cores)
...
benchmark $DLW_param ([ref]$dataDLW_latency) ([ref]$dataDLW_iops) ([ref]$dataDLW_cores)
Remove-Item $TestFilePath
...
benchmark $LRW_param ([ref]$dataLWR_latency) ([ref]$dataLWR_iops) ([ref]$dataLWR_cores)
Remove-Item $TestFilePath
